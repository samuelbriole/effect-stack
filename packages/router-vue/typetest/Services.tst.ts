...
import { createRootRoute, createRoute, createRouter } from "@effect-stack/router-vue"
...
const projects = Layer.effect(
  Projects,
  Connection.use((service) => Effect.succeed(Projects.of({ title: service.name })))
)
...
describe("Vue Effect Layer requirements", () => {
  test("combines ancestor and lazy services and rejects unresolved Layer dependencies", () => {
