...
import { createRootRoute, createRoute, createRouter, type Destination, RouterProvider } from "@effect-stack/router-vue"
...
declare const match: ReturnType<typeof project.useMatch>["value"]
...
describe("Vue route inference", () => {
  test("preserves inherited paths and readonly computed refs", () => {
    expect(layout.to).type.toBe<"/">()
    expect(project.to).type.toBe<"/projects/:id">()
    expect<ReturnType<typeof project.useParams>>().type.toBe<ComputedRef<{ readonly id: typeof ProjectId.Type }>>()
    expect<ReturnType<typeof project.useSearch>>().type.toBe<ComputedRef<{ readonly tab?: string }>>()
    expect<ReturnType<typeof project.useLoaderData>>().type.toBe<
      ComputedRef<{ id: typeof ProjectId.Type; name: string }>
    >()
  ...
  test("requires native Layers and ranked index URL inputs", () => {
    ...
    expect(RouterProvider).type.toBeCallableWith({ router })
    expect(RouterProvider).type.not.toBeCallableWith({ router: tree })
