...
describe("Effect service injection", () => {
  ...
  it.effect(
    "shares a composed Layer across ancestor and child loaders and refreshes, then releases it with the registry",
    () =>
      Effect.gen(function* () {
        let acquired = 0
        let released = 0
        ...
        expect(
          registry
            ...
            .matches.map((match) => (match.result._tag === "Success" ? match.result.value.loaderData : undefined))
